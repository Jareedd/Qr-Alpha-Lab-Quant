...
**In-flight result to fold in:** `scripts/pbo_equity.py` was running in the background — the real PBO over the **comparable equity-config family (#2/#3/#5/#6/#7) on the shared PIT return matrix**, run as a REPRODUCTION (no trial spent). When it lands, fold `results/pbo_equity.json` into the write-up and add a `research_log.md` row. **PBO is scoped FAMILY-WISE on a shared return matrix — NOT across the 11 heterogeneous trials** (doing the latter would be exactly the overfitting error the metric exists to catch).
...
4. **Fold in the pending PBO equity result.** Check whether the background `scripts/pbo_equity.py` run produced `results/pbo_equity.json`; if so, add a `research_log.md` row (REPRODUCTION, no trial spent) and wire the number into `writeup/research_note.md`. Keep PBO scoped family-wise (#2/#3/#5/#6/#7 on the shared return matrix), never across the 11 heterogeneous trials.
